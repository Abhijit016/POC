...
public class CouchbaseConfig extends AbstractCouchbaseConfiguration {
    ...
    protected void configureRepositoryOperationsMapping(RepositoryOperationsMapping mapping){
        try {
            mapping.mapEntity(Module.class, getCouchbaseTemplate("module"));
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    private CouchbaseTemplate getCouchbaseTemplate(String bucketName) throws Exception {
        CouchbaseTemplate template = new CouchbaseTemplate(couchbaseClientFactory(bucketName), mappingCouchbaseConverter(couchbaseMappingContext(customConversions()),
